...
function(i) {
    ...
    h = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>', '<div class="layim-chat-user"><img src="'+httpurlIm+'{{ d.avatar }}"><cite>', "{{# if(d.mine){ }}", '<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"佚名" }}', "{{# } else { }}", '{{ d.username||"佚名" }}<i>{{ layui.data.date(d.timestamp) }}</i>', "{{# } }}", "</cite></div>", '<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>', "</li>"].join(""),
    v = '<li class="layim-{{ d.data.type }}{{ d.data.id }} layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="'+httpurlIm+'{{ d.data.avatar }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>',
    g = function(i) {
        return i < 10 ? "0" + (0 | i) : i
    };
    ...
    layui.data.content = function(i) {
        ...
        function(i) {
            return '<img class="layui-layim-photos" src="' + httpurlIm + i.replace(/(^img\[)|(\]$)/g, "") + '">'
        }).replace(/file\([\s\S]+?\)\[[\s\S]*?\]/g,
        function(i) {
            var a = (i.match(/file\(([\s\S]+?)\)\[/) || [])[1],
            e = (i.match(/\)\[([\s\S]*?)\]/) || [])[1];
            return a ? '<a class="layui-layim-file" href="' + httpurlIm + a + '" download target="_blank"><i class="layui-icon">&#xe61e;</i><cite>' + (e || a) + "</cite></a>": i
        }).replace(/audio\[([^\s]+?)\]/g,
        function(i) {
    ...
    T = function(i) {
        ...
        t.open({
            ...
            offset: "rb",
            resize: !1,
            content: '<img src="'+ httpurlIm + (j.mine.avatar || layui.cache.dir + "css/pc/layim/skin/logo.jpg") + '"><span>' + (i || j.base.title || "我的通讯") + "</span>",
    ...
    N = function(i) {
        ...
        N.index = t.open({
            ...
            resize: !1,
            area: ["182px", "50px"],
            content: '<img id="layui-layim-min" src="'+ httpurlIm + a.avatar + '"><span>' + a.name + "</span>",
        ...
        search: function(i) {
            ...
            n = function(i) {
                ...
                if ("" === a) ti.tab(0 | ti.tab.index);
                    ...
                    if (n.length > 0) for (var u = 0; u < n.length; u++) o += '<li layim-event="chat" data-type="' + n[u].type + '" data-index="' + n[u].index + '" data-list="' + n[u].list + '"><img src="'+ httpurlIm + n[u].avatar + '"><span>' + (n[u].username || n[u].groupname || "佚名") + "</span><p>" + (n[u].remark || n[u].sign || "") + "</p></li>";
            ...
            i.data("down") ? l() : (n.html("&#xe619;"), i.data("down", !0), ti.groupMembers.index = t.tips('<ul class="layim-members-list"></ul>', i, {
                ...
                success: function(a) {
                    ...
                    function(a) {
                        layui.each(a.list,
                        function(i, a) {
                            o += '<li data-uid="' + a.id + '"><a href="javascript:;"><img src="'+ httpurlIm + a.avatar + '"><cite>' + a.username + "</cite></a></li>",
                            d[a.id] = a
                        }),
                        s.html(o),
